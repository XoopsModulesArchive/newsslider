...
function b_news_glider_show( $options ) {
    ...
    if ($options[23] == 0) {
        $stories = $tmpstory->getAllPublished($options[0],0,$restricted,0,1, true, $options[13]);
    } else {
    ...
    unset($tmpstory);
      ...
      foreach ( $stories as $story ) {
        ...
        if ($options[16] > 0) {
          ...
          $br = $options[22] == 1 ? 0 : 1;
          if ($module->getVar('version') <= 160) { //--- for News versions prior to 1.60
            $news['teaser'] = xoops_substr($myts->displayTarea(strip_tags($story->hometext)), 0, $options[16]+3);
          } else { //--- for News version 1.60+
